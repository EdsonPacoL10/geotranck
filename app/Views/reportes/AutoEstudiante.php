    ...
    <main>
        ...
        <div id="contenido">
            <table class="styled-table" width="100%">
                ...
                <tr align="left">
                    <td colspan="2"><b>Marca:</b> <?= esc($AutoEstudiante[0]['marca']) ?> <b>Color:</b>
                        <?= esc($AutoEstudiante[0]['color']) ?> <b>Modelo:</b>
                        <?= esc($AutoEstudiante[0]['modelo']) ?> <b>Placa:</b> <?= esc($AutoEstudiante[0]['placa']) ?>
                    </td>
                </tr>
            </table>
        </div>
        <div id="contenido">
            <table class="styled-table" width="100%">
                ...
                <tr align="left">
                    <td width="30%"><b>Nombre y Apellido:</b></td>
                    <td><?= esc($AutoEstudiante[0]['nom_conductor']) ?>
                        <?= esc($AutoEstudiante[0]['ape_conductor']) ?></td>
                </tr>
                <tr align="center">
                    <td colspan="2"><b>Teléfono:</b> <?= esc($AutoEstudiante[0]['tel_conductor']) ?></td>
                </tr>
            </table>
        </div>
        <?php if (!empty($AutoEstudiante)): ?>
            <table border="1" class="styled-table">
                ...
                <tbody>
                    <?php $contador = 1; ?>
                    <?php foreach ($AutoEstudiante as $dato): ?>
                        <tr>
                            <td width="5%"><?= $contador++ ?></td>
                            <td width="20%"><?= esc($dato['nom_padre']) ?>  <?= esc($dato['ape_padre']) ?></td>
                            <td width="20%"><?= esc($dato['tel_padre']) ?><br><b>Parentesco:</b> <?= esc($dato['parentesco']) ?></td>
                             <td width="15%"><?= esc($dato['nom_estudiante']) ?> <?= esc($dato['ape_estudiante']) ?></td>
                            <td width="20%"><?= esc($dato['curso']) ?></td>
